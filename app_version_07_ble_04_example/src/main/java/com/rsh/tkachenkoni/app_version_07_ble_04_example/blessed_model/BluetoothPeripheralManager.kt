...
import com.rsh.tkachenkoni.app_version_07_ble_04_example.blessed_model.BluetoothBytesParser.Companion.mergeArrays
import java.util.*
...
class BluetoothPeripheralManager(
    ...
    callback: BluetoothPeripheralManagerCallback
) {
    ...
    val bluetoothGattServer: BluetoothGattServer
    private val callback: BluetoothPeripheralManagerCallback
    protected val commandQueue: Queue<Runnable> = ConcurrentLinkedQueue()
    private val writeLongCharacteristicTemporaryBytes =
        HashMap<BluetoothGattCharacteristic, ByteArray>()
    ...
    protected val bluetoothGattServerCallback: BluetoothGattServerCallback =
        object : BluetoothGattServerCallback() {
            ...
            private fun handleDeviceConnected(device: BluetoothDevice) {
                ...
                connectedCentralsMap[bluetoothCentral.getAddress()] = bluetoothCentral
            ...
            private fun handleDeviceDisconnected(device: BluetoothDevice) {
                ...
                Logger.i(
                    TAG,
                    "Central '%s' (%s) disconnected",
                    bluetoothCentral.getName(),
                    bluetoothCentral.getAddress()
                )
            ...
            override fun onCharacteristicReadRequest(
                ...
                Logger.i(
                    TAG,
                    "read request for characteristic <%s> with offset %d",
                    characteristic.uuid,
                    offset
                )
                val bluetoothCentral = getCentral(device)
                mainHandler.post { // Call onCharacteristic before any responses are sent, even if it is a long read
                    ...
                    val value = copyOf(
                        nonnullOf(characteristic.value),
                        offset,
                        bluetoothCentral.getCurrentMtu() - 1
                    )
                ...
                mainHandler.post {
                    ...
                    if (!preparedWrite) {
                        ...
                        if (offset == 0) {
                            ...
                            val temporaryBytes =
                                writeLongCharacteristicTemporaryBytes[characteristic]
                            if (temporaryBytes != null && offset == temporaryBytes.size) {
                                writeLongCharacteristicTemporaryBytes[characteristic] =
                                    mergeArrays(temporaryBytes, safeValue)
                            } else {
            ...
            override fun onDescriptorReadRequest(
                ...
                Logger.i(
                    TAG,
                    "read request for descriptor <%s> with offset %d",
                    descriptor.uuid,
                    offset
                )
                val bluetoothCentral = getCentral(device)
            ...
            override fun onDescriptorWriteRequest(
                ...
                val characteristic = Objects.requireNonNull(
                    ...
                    "Descriptor does not have characteristic"
                )
                Logger.i(
                    TAG,
                ...
                mainHandler.post {
                    ...
                    if (descriptor.uuid == CCC_DESCRIPTOR_UUID) {
                        ...
                        if (!preparedWrite) {
                            ...
                            status =
                                callback.onDescriptorWrite(bluetoothCentral, descriptor, safeValue)
                        } else {
                            if (offset == 0) {
                                ...
                                if (temporaryBytes != null && offset == temporaryBytes.size) {
                                    writeLongDescriptorTemporaryBytes[descriptor] =
                                        mergeArrays(temporaryBytes, safeValue)
                                } else {
                                    status = GattStatus.INVALID_OFFSET
                                }
            ...
            private fun checkCccDescriptorValue(
                safeValue: ByteArray,
                characteristic: BluetoothGattCharacteristic
            ): GattStatus {
                var status = GattStatus.SUCCESS
                ...
                } else if (!supportsIndicate(characteristic) && Arrays.equals(
                        safeValue,
                        BluetoothGattDescriptor.ENABLE_INDICATION_VALUE
                    )
                ) {
                    status = GattStatus.REQUEST_NOT_SUPPORTED
                } else if (!supportsNotify(characteristic) && Arrays.equals(
                        safeValue,
                        BluetoothGattDescriptor.ENABLE_NOTIFICATION_VALUE
                    )
                ) {
                ...
                if (execute) {
                    mainHandler.post {
                        ...
                        if (!writeLongCharacteristicTemporaryBytes.isEmpty()) {
                            val characteristic =
                                writeLongCharacteristicTemporaryBytes.keys.iterator().next()
                            if (characteristic != null) {
                                // Ask callback if value is ok or not
                                status = callback.onCharacteristicWrite(
                                    bluetoothCentral, characteristic,
                                    writeLongCharacteristicTemporaryBytes[characteristic]!!
                                )
                                if (status === GattStatus.SUCCESS) {
                            ...
                            val descriptor =
                                writeLongDescriptorTemporaryBytes.keys.iterator().next()
                            if (descriptor != null) {
                                // Ask callback if value is ok or not
                                status = callback.onDescriptorWrite(
                                    bluetoothCentral, descriptor,
                                    writeLongDescriptorTemporaryBytes[descriptor]!!
                                )
                                if (status === GattStatus.SUCCESS) {
            ...
            override fun onNotificationSent(device: BluetoothDevice, status: Int) {
                val bluetoothCentral = getCentral(device)
                val characteristic = Objects.requireNonNull(currentNotifyCharacteristic)
                val value = Objects.requireNonNull(currentNotifyValue)
    ...
    protected val advertiseCallback: AdvertiseCallback = object : AdvertiseCallback() {
     ...
     */
    fun getServices(): kotlin.collections.List<BluetoothGattService> {
        return bluetoothGattServer.services
    }
    ...
    private fun notifyCharacteristicChanged(
        ...
        val result = commandQueue.add(Runnable {
            ...
            if (!bluetoothGattServer.notifyCharacteristicChanged(
                ...
                Logger.e(
                    TAG,
                    "notifying characteristic changed failed for <%s>",
                    characteristic.uuid
                )
                completedCommand()
            }
    ...
    private fun getConnectedDevices(): kotlin.collections.List<BluetoothDevice> {
     ...
     */
    fun getConnectedCentrals(): kotlin.collections.Set<BluetoothCentral> {
        val bluetoothCentrals: kotlin.collections.Set<BluetoothCentral> =
            HashSet(connectedCentralsMap.values)
    ...
    private fun cancelAllConnectionsWhenBluetoothOff() {
        val bluetoothCentrals = getConnectedCentrals()
